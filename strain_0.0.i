...
[./ca] #Global Composition of A for ternary A-B-C alloy; A= Al
  variable = ca
  type = FunctionIC
  function = 'r1:=sqrt((x-225)^2+0.5*(y-165)^2); r2:=sqrt((x-285)^2+(y-420)^2); r3:=sqrt((x-200)^2+(y-320)^2);r4:=sqrt((x-420)^2+(y-400)^2);r5:= sqrt(0.6*(x-380)^2+(y-180)^2);  if(r1<=55,0.66,if(r2<=65,0.66,if(r3<=50,0.641,if(r4<=60,0.641,if(r5<=70,0.641,0.06)))))'
  [../]
...
############################################  
 [./cb] #Global Composition of B for ternary A-B-C alloy; B = Ni
  variable = cb
  type = FunctionIC
  function = 'r1:=sqrt((x-225)^2+0.5*(y-165)^2); r2:=sqrt((x-285)^2+(y-420)^2); r3:=sqrt((x-200)^2+(y-320)^2);r4:=sqrt((x-420)^2+(y-400)^2);r5:= sqrt(0.6*(x-380)^2+(y-180)^2);  if(r1<=55,0.32,if(r2<=65,0.32,if(r3<=50,0.349,if(r4<=60,0.349,if(r5<=70,0.349,0.04)))))'
  [../]                
[]
########################################################################################
...
[./fch3]  # Al rich matrix eta6
  ...
  args = 'c3a c3b'
  function = '(energy_scale/(length_scale)^3) *(-13.22 + 400*(c3a-0.06)^2+ 643*(c3b-0.04)^2 )*factor_f3/molar_vol' 
  outputs = exodus
[../]
